	...
	def __init__(self, encode_dims=[], decode_dims=[], dropout=0.0):
		...
		self.decoder = nn.ModuleList({
			f'dec_{i}': nn.Linear(decode_dims[i], decode_dims[i+1])
				for i in range(len(decode_dims)-2)
		})
		...
		def inference(self, x):
			mu, log_var = self.encode(x) # 为什么没有用
		...
		def decode(self, z):
			hid = z
			for i, (name, layer) in enumerate(self.decoder.items()):
				hid = layer(hid)
...
if __name__ == '__main__':
	...
	model = model.cuda()
	input_ = torch.randn(234, 1024).cuda()
	output, mu, log_var = model(input_)
	print(output,shape)
	print(mu.shape)
